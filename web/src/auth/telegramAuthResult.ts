// Виджет Telegram на мобильном открывает не попап, а новую вкладку — тот же
// window.open(popup_url) в telegram-widget.js, но мобильный браузер не умеет
// класть окна друг на друга и уводит на новую вкладку. Telegram подтверждает
// вход и редиректит эту вкладку обратно на return_to с результатом во
// фрагменте адреса, а не через postMessage: исходная вкладка (там, где
// осталась кнопка со спиннером) этого сообщения не дождётся никогда — баг с
// прода, найден владельцем 2026-09-08. Разбор фрагмента — тот же алгоритм,
// что у `haveTgAuthResult()` внутри самого telegram-widget.js: regex по
// `location.hash`, base64url → base64 с паддингом, декодирование в JSON —
// с одной поправкой на кириллицу, см. decodeBase64UrlJson ниже. Подлинность
// мы не проверяем — её проверяет сервер той же подписью `hash`,
// что и у пути через попап (SECURITY.md §2): битый или подделанный фрагмент
// не даёт новых прав, только не должен ронять экран входа — поэтому любое
// отклонение от формы возвращает `null`, а не бросает исключение.
