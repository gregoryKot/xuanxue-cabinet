...
describe('useGradedAttempts — загрузка', () => {
  it('запрашивает проверенные работы с лимитом 200', async () => {
    mockedApiFetch.mockResolvedValueOnce([]);
    const { result } = renderHook(() => useGradedAttempts());
  ...
  it('не-ApiError — общий текст, не сырое сообщение', async () => {
    ...
    await waitFor(() =>
      expect(result.current.error).toBe(
        'Не удалось загрузить проверенные работы. Попробуйте ещё раз.',
      ),
    );
  });
