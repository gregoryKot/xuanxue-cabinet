...
export class ExamAttemptsService {
   ...
   *
   * Срок сдачи (`dueAt`, ADR-0125) закрывает только путь «завести НОВУЮ» —
   * стоит после возврата уже идущей (ветка `existing`) и никогда не раньше:
   * решение владельца 2026-09-22, идущую попытку срок не прерывает. До
   * подсчёта `attemptsUsed`, не после: прошедший срок — то же «нет» при
   * любом числе попыток, причина ученику точнее и без лишнего запроса. */
