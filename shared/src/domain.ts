...
/** Кому писать человеку, которого в школе ещё нет: бот называет этот контакт
 * незнакомцу (`buildStrangerMessage`, ADR-0115) — школа маленькая, новичка
 * заводит Дима руками, и ссылки-приглашения у незнакомца нет и быть не должно
 * (ADR-0030). Как и `DEFAULT_PREVIEW_MINUTES`, это дефолт для базы без поля, а
 * не источник правды: значение живёт в БД и на экране «Шаблоны», иначе смена
 * контакта потребовала бы разработчика (CLAUDE.md «Кабинет учителя: всё
 * настраивается в интерфейсе»). Свободный текст, а не один `@ник`: писать
