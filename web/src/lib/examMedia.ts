...
/** «12 сентября, 19:30, 3 мин 40 с» — только момент и длительность, без
 * слова «получено»: блок ответа ученика (attempt/AttemptVideoAnswerRow.tsx)
 * уже назвал строкой выше, ЧТО пришло, и повторять это в подписи снизу
 * незачем. `timeZone` — как у `formatDateTime`, нужен только тесту для
 * детерминизма; сами экраны зовут без него — пояс браузера, тот же приём,
 * что у остальных дат кабинета (GradingQueueCard.tsx). */
export function formatExamMediaWhen(media: ExamMediaDto, timeZone?: string): string {
 ...
 * строкой для карточки проверки учителя (grading/AttemptReviewMedia.tsx).
 * Собирается из formatExamMediaWhen, а не считает время во второй раз. */
export function formatExamMediaReceivedAt(
  media: ExamMediaDto,
  timeZone?: string,
): string {
  return `Видео получено ${formatExamMediaWhen(media, timeZone)}`;
}
