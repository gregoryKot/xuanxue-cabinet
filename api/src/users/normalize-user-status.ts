...
// (ADR-0034, миграция 0007-invited-users-active). Миграция одноразовая и
...
// значение вне enum).
//
// Решение владельца (ADR-0034, вариант «б»): неизвестный статус на чтении —
...
export function normalizeUserStatus(raw: unknown, userId: string): UserStatus {
  ...
  logger.warn(
    `user.status.unknown: статус «${String(raw)}» у пользователя ${userId} прочитан как active (ADR-0034, expand→contract)`,
  );
  return 'active';
}
