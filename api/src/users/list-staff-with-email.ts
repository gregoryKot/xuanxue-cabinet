...
// отличие от listTeacherContacts.ts (`telegramId: { $exists: true }`,
...
// файлом тем же приёмом, что list-teacher-contacts.ts: UsersService не растёт
// за 150 строк (CLAUDE.md «Храповики»).
...
export async function listStaffWithEmail(
  ...
  const docs = await model
    ...
    // Тот же лимит-гейт, что у listTeacherContacts — «дай всё» запрещено
