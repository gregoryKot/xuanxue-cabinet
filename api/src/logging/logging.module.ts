...
// прокси/балансировщиком), иначе генерируем (incomingRequestId,
// common/request-info.ts — общая логика с raw-upload-concurrency.ts) — и
// всегда кладём тот же id в заголовок ответа, чтобы клиент мог сослаться на
// него в баг-репорте.
function genReqId(req: IncomingMessage, res: ServerResponse): string {
  const id = incomingRequestId(req.headers[REQUEST_ID_HEADER]);
