...
// Отзыв владельца 2026-09-22, вторая итерация («нижняя панель всё ещё не та,
// очень большая, внизу много пустого пространства») — дополнение к ADR-0110
// в docs/adr/0110-phone-tabs-stand-on-the-same-paper.md. Панель на общей
// бумаге сделала системную зону под подписями видимой пустотой (замер
// владельца: 86px вся панель, 43px — пустота под подписью). Отступ снизу
// теперь ограничивает вклад env(safe-area-inset-bottom) потолком, а не
// складывает его целиком. jsdom calc() не вычисляет, поэтому гейт сверяет не
// число, а саму строку стиля — она и есть источник правды для браузера.
describe('AppNav — нижний отступ панели ограничивает безопасную зону потолком (ADR-0110, доп. 2026-09-22)', () => {
  it('paddingBottom складывает отступ панели и min() безопасной зоны с потолком', () => {
    renderNav(true, TEACHER, '/planning');

    const nav = screen.getByRole('navigation', { name: 'Разделы кабинета' });
    // Строка, а не отдельные числа: calc()/env()/min() в jsdom не считаются,
    // браузер получает именно то, что здесь записано символ в символ.
    expect(nav.style.paddingBottom).toBe(
      'calc(4px + min(env(safe-area-inset-bottom), 20px))',
    );
  });
});
