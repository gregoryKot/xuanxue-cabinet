// Подтверждение перед необратимым действием страницы-редактора (удалить
// экзамен или вопрос-черновик, отменить занятие) — общая механика с
// components/ConfirmDialog.tsx (CLAUDE.md «Одна механика — один компонент»):
// открыть диалог, дождаться ответа сервера, уйти со страницы только при
// успехе и только после того, как сам диалог подтверждения уже закрылся.
// Раньше «Удалить» у экзамена и вопроса банка срабатывало с одного касания
// без подтверждения и отмены — на телефоне рядом с ней другие кнопки (аудит
// 2026-09-15, важно №2). Вынесено из формы канала (там та же связка
// confirming/removed+эффект была написана вручную): вторая и третья копии
// одного и того же — сюда, а не по файлам (jscpd, CLAUDE.md «Дубли и мёртвый
// код»).
...
export function useConfirmedRemove(
  ...
  // Не уходим со страницы прямо в confirmRemove: ConfirmDialog после этого
