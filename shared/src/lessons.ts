...
export interface LessonDto {
   ...
   * для него есть название и `groupLabel` (ADR-0070, уточняет ADR-0058).
...
export interface ListLessonsQuery {
  ...
  to: string;
  classId?: string;
  /** Точное совпадение тега — как у `ListMaterialsQuery.tag` (ADR-0070). */
  tag?: string;
  limit?: number;
}
...
export interface MyLessonDto {
   ...
   * `MyMaterialDto.tags`; фильтра по тегу тут нет (ADR-0070). */
  tags: string[];
}
