...
// Бумага направления «тихо и благородно» — фон страницы кабинета (--paper в
// web/src/index.css, docs/adr/0031-visual-direction-quiet-and-noble.md).
// Манифест собирается из этого файла, до CSS-переменной ему не дотянуться,
// поэтому тон продублирован числом; расходится он только вместе с правкой
// палитры, и тогда правится здесь же.
const PAPER = '#faf8f4';
// Тон статус-бара и адресной строки. Шапка кабинета залита бумагой
// (web/src/app/AppShell.tsx), поэтому и статус-бар над ней — бумага: он
// продолжает шапку, а не режет её полосой чужого цвета. Киноварь сюда не
// берём — акцент на экране один раз, и он уже занят знаком-печатью (ADR-0031).
const THEME_COLOR = PAPER;
// Фон сплэш-экрана при запуске установленного приложения — та же бумага, что
// фон первого экрана: между сплэшем и кабинетом нет белой вспышки.
const BACKGROUND_COLOR = PAPER;
