// По образцу useMyLessons.test.ts — тонкая обёртка над useAbortableFetch,
...
describe('useMyArchive — загрузка', () => {
  it('запрашивает /me/lessons/archive без своего лимита, отдаёт список', async () => {
    const lessons = [{ id: 'l1' }];
    mockedApiFetch.mockResolvedValueOnce(lessons);

    const { result } = renderHook(() => useMyArchive());
    await waitFor(() => expect(result.current.loading).toBe(false));

    expect(mockedApiFetch).toHaveBeenCalledWith('/me/lessons/archive', expect.anything());
