...
describe('Exam attempts (e2e)', () => {
  ...
  // (useAttempt.ts), и ответ обязан быть уже там, а не только в ответе на
  // сам PATCH (read-after-write, CLAUDE.md «Тесты»).
  it('ученик: сохранённый ответ виден в свежем GET /attempts — переживает закрытие вкладки', async () => {
    const teacherCookie = await sessionFor(['teacher']);
