...
Первый заезд экзамена «Форма 1» в базу делает не эта команда, а миграция
`0013-exam-form-1` сама при деплое (ADR-0064, дополнение 2026-09-20) — локально
запускать её не нужно. `seed:exam` остаётся для повторного импорта и вопросов,
дописанных в файл позже, тем же способом, только файл лежит в репозитории открыто
(секретов в нём нет), см. RUNBOOK §2.3:
