...
// тот же чеклист CLAUDE.md «Новая коллекция», что у
...
// (ADR-0037, было — одна на попытку). С ADR-0084 частичный уникальный индекс
// ниже больше не запрещает вторую ссылку — повторная отправка заменяет
...
// считается той же «пустой» связкой у Mongo. С ADR-0084 индекс не запрещает
// вторую ссылку на ту же связку — upsertLinkMediaAsset пишет по нему поверх
// прежней записи; конфликт остаётся только у гонки двух параллельных upsert.
