...
export interface BroadcastInsertPayload {
  ...
  scheduledAt: Date;
  text: string;
  /** Раннер не берёт доставку раньше этого момента: у разовой рассылки
   * (scheduledAt учителя может быть в будущем) и у ссылки на занятие (момент
   * реальной отправки — startsAt минус leadMinutes, документ мог быть создан
   * раньше ради предпросмотра, PLAN.md §6) это будущее время; у записи это
   * тот же момент, что и сама рассылка — раннер должен забрать сразу, поле
   * не заполняется. */
