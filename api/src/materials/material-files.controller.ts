...
// по тому же `isMaterialLocked`, что и список (ADR-0048).
import {
  Controller,
  Delete,
  Get,
...
export class MaterialFilesController {
  ...
  // библиотеке (`/me/materials`). Закрытый по оплате не откроется никому,
  // кроме штата, — это решает сервис, а не декоратор.
  @Get(':id/file')
  async download(
    @Param('id') id: string,
