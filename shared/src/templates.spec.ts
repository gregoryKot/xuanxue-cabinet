...
describe('renderTemplate — плейсхолдеры и фрагменты', () => {
  ...
  it('незакрытые скобки подряд — весь хвост остаётся текстом как есть', () => {
    // Регресс аудита M9: раньше при отсутствии `]` цикл всё равно продолжал
    // пересканировать хвост на каждой следующей `[`, что квадратично на
    // '['.repeat(n) — сам факт остаётся текстом должен не измениться.
    expect(renderTemplate('[[[[', {})).toBe('[[[[');
    expect(renderTemplate('[a] [b [c', {})).toBe(' [b [c');
  });

  it('линейная сложность парсера на несбалансированных `[` (аудит M9)', () => {
    // Не таймер, а факт результата: при квадратичном разборе этот вызов
    // занимал бы секунды на 200k символов, при линейном — миллисекунды,
    // но детерминированный тест проверяет только правильность ответа
    // (CLAUDE.md, «Тесты», принцип детерминизма — время не измеряем).
    const template = '['.repeat(200_000);
