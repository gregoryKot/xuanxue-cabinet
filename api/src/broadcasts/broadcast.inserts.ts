...
export interface BroadcastInsertPayload {
   ...
   * (scheduledAt учителя в будущем) и у ссылки на занятие (startsAt минус
   * leadMinutes — документ создан раньше ради предпросмотра, PLAN.md §6) это
   * будущее время; у записи — момент самой рассылки, поле не заполняется. */
