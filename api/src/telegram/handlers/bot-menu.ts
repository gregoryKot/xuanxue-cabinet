...
// Отказ незнакомцу называет путь внутрь, а не аудиторию бота (ADR-0115,
...
const STRANGER_MESSAGE_BASE =
  'Это бот школы Сюань-Сюэ: он присылает ссылки на занятия и принимает ' +
  'экзамены. Вас он пока не узнаёт. Откройте кабинет, зайдите в «Профиль» ' +
  'и нажмите «Связать Telegram» — после этого пишите сюда.';
const MENU_TEXT =
 ...
 * на всех незнакомцев, знаком нам этот telegramId или нет (SECURITY §2).
 * Адрес школы — для того, кто в школе ещё не занимается: кабинета у него
 * нет, и совет про «Связать Telegram» ему ничего не даёт. */
export function buildStrangerMessage(schoolSiteUrl?: string): string {
  if (!schoolSiteUrl) return STRANGER_MESSAGE_BASE;
  return `${STRANGER_MESSAGE_BASE} Ещё не занимаетесь в школе? Расписание — на сайте ${schoolSiteUrl}`;
}
