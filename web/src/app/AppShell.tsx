...
export function AppShell() {
  ...
  // один компонент»). `me === null` — роль ещё не известна: в продакшене
  // сюда не попасть (RequireAuth держит на скелетоне, пока status
  // 'loading'), но `<Navigate>`, в отличие от прежней подмены компонента, —
  // настоящая запись в историю и не отменяется сама, если решение принято
  // вслепую. Поэтому редирект ждёт настоящего `me`, не гадает по пустому.
  const canSee = me === null || canSeeRoute(me, pathname);
