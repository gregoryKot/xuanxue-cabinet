...
interface MyExamAttemptSummaryDto {
   ...
   * самой попытки (ADR-0120, describeExamTime в exam-time.ts). */
  deadlineAt?: string;
...
export interface MyExamDto {
   ...
   * и про время экран не говорит ничего (ADR-0120). Не срок сдачи: своего
   * «сдать до такого-то числа» у экзамена в модели нет. */
  timeLimitMin?: number;
