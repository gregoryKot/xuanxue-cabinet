...
//
// icon.svg — печать киноварью с вырезанным в ней тайцзи (ADR-0031), без
// <text> (в CI-рендере sharp нет CJK-шрифтов, символ — только path/circle).
// Отсюда четыре файла:
//   icon-192.png, icon-512.png    — обычная иконка (manifest purpose "any"),
//                                    фон вне печати прозрачный.
...
// Фон там, где прозрачность недопустима (маска Android, флатенинг iOS), —
// киноварь самой печати из icon.svg: маска режет сплошную плашку и не
// оставляет шва другого цвета по краю. С theme_color манифеста
// (web/vite.config.ts) этот тон намеренно разошёлся: статус-бар продолжает
// шапку кабинета и залит бумагой, а иконка на экране «Домой» узнаётся
// пятном киновари.
const SOLID_BG = '#9c4221';
