...
// Совета «напишите учителю, он поправит» здесь больше нет: механизма правки
// не было ни у ученика, ни у учителя, а правило без механизма не работает
// (CLAUDE.md). Осталось то, что правда, и действие, которое ученику доступно
// прямо сейчас. Запись без itemId (старый инстанс на деплое, expand →
// contract) попадает в тот же отказ — «этот вопрос» для неё неточно, но живёт
// такая запись один деплой.
export const EXAM_MEDIA_ALREADY_LINKED_MESSAGE =
  'К этому вопросу ссылка уже прикреплена — учитель её видит. Проверьте, открывается ли она без входа в ваш аккаунт.';
