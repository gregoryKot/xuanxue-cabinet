...
// самом ExamItemsService (длина формулировки/варианта, размер массива
// вариантов — всё живёт в декораторах DTO), бот обязан проверить той же
// проверкой, не второй копией лимитов (CLAUDE.md «Одна механика — один
// компонент»). Вынесено из exam-bot.service.ts (файл-лимит 150 строк).
...
/** `skipUndefinedProperties` — черновик неполон до последнего шага (часто нет
 * ещё вариантов): непереданное поле не должно считаться пустым `''`/`[]` и
 * падать на `@IsNotEmpty()`/`@ArrayMaxSize()`, которых при частичном шаге ещё
 * рано спрашивать. `null` — ошибок нет. */
