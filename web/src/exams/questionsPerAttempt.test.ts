...
describe('questionsPerAttemptHint', () => {
  ...
  it('непустой список — называет число и объясняет отметку ★', () => {
    expect(questionsPerAttemptHint(56)).toBe(
      'Пусто — каждый ученик отвечает на все 56 вопросов списка. Впишите число — ' +
        'и каждому ученику достанется столько случайных вопросов из 56, у каждого ' +
...
describe('questionsListNote', () => {
  it('называет число, доступное сдающему, и общий размер списка', () => {
    expect(questionsListNote(15, 50)).toBe(
      'Каждому ученику достанется 15 из 50, случайно. ★ — обязательные, попадут всем.',
    );
  });
});
