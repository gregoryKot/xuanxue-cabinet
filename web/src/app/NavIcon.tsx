// Значки нижней панели телефона (ADR-0097) — по одному на раздел, под каждым
// стоит его подпись словом (ADR-0103). Значок decorative-only: имя ссылке даёт
// подпись, и озвучь скринридер ещё и значок — раздел назывался бы дважды. Тот
// же приём, что ProfileIcon.tsx и BellIcon.tsx: свой inline-SVG без иконной
// библиотеки (CLAUDE.md «Зависимости»). Один компонент на все имена, не файл на
// значок — `SHAPES` типа `Record<NavIconName, ReactNode>` не даёт `tsc`
// пропустить новое имя без рисунка.
import type { CSSProperties, ReactNode } from 'react';
import type { NavIconName } from './navItems';

// В панели значок крупнее, чем 20px у ProfileIcon/BellIcon в верхней строке:
// там он стоит рядом с текстом в его кегль, здесь — над подписью в 10 кеглей и
// опознаётся первым, до чтения слова. Больше 22 брать некуда: столбец «значок +
// подпись» считан под цель нажатия 44px (bottomNavStyles.ts).
