...
// к Mongo на каждое открытие архива). Правило доступа по оплате (ADR-0048) и
// вырезание `access: 'staff'` (ADR-0058) работают тем же приёмом и на тех же
// функциях, что и MaterialsService.listForStudent — иначе рубильник школы
// обходился бы через архив, а служебный материал утекал бы ученику.
...
export class LessonMaterialsService {
   ...
   * ADR-0048): сервис решает по роли, не по личности. Возвращает материалы
  ...
  async findByLessonIds(
    ...
    for (const doc of docs) {
      ...
      // ГЛАВНОЕ (ADR-0048): рубильник платного доступа действует и в архиве,
      // ровно как в библиотеке — иначе его можно обойти открыв не
      // «Материалы», а «Архив занятий».
      const locked = isMaterialLocked({
        access: decrypted.access,
        paidAccessEnabled: settings.materialsPaidAccess,
        isStaff,
      });
      const dto = toMyMaterialDto(decrypted, classTitleById, locked);
