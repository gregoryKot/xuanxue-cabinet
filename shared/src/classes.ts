...
export interface ClassDto {
  ...
  leadMinutes: number;
  active: boolean;
  /** Постоянная рубрика курса — «начинающие», «медитация» (ADR-0070).
   * Тег этого вечера живёт у даты (`LessonDto.tags`, ADR-0059) и не
   * подмешивается сюда: одно написание — одно место. У занятий, заведённых
   * до ADR-0070, поля в документе нет — маппер отдаёт `[]`. */
  tags: string[];
  createdAt: string;
...
export interface UpdateClassInput {
   ...
   * NULLABLE_CLASS_FIELDS поле не входит (ADR-0070). */
  tags?: string[];
}
